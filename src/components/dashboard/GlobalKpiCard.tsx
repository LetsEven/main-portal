...
const GlobalKpiCard: React.FC<GlobalKpiCardProps> = ({
  ...
  const getCardType = () => {
    ...
    if (title === "Ingresos Even") return "revenue";
  ...
  const getMetricType = () => {
    if (title.includes("Volumen") || title.includes("Ingresos"))
      return "currency";
    if (
      title.includes("Tasa") ||
      title.includes("Conversión") ||
      title.includes("Aprobación")
    )
      return "percent";
    return "number";
  };
  ...
  return (
    <>
      <div
        className={`bg-white rounded-lg p-3 sm:p-4 border border-gray-100 hover:shadow-md transition-shadow ${shouldShowDetailedChart() ? "cursor-pointer" : ""}`}
      ...
      >
        <div className="flex justify-between items-start gap-1">
          <div className="flex items-center min-w-0 flex-1">
            {icon && (
              <span className="mr-1 sm:mr-1.5 flex-shrink-0">{icon}</span>
            )}
          ...
          {!hideChange && (
            <div
              className={`text-[10px] sm:text-xs font-medium px-1 sm:px-1.5 py-0.5 rounded flex-shrink-0 ${getChangeColor()}`}
            >
              {formatChange(change)}
            </div>
          )}
