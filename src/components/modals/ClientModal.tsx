...
const ClientModal: React.FC<ClientModalProps> = ({
  ...
  const validateForm = (): boolean => {
    ...
    const requiresTableCount =
      formData.services.includes("flex-bill") ||
      formData.services.includes("tap-order-pay");
    if (requiresTableCount) {
      if (!formData.tableCount || formData.tableCount < 1) {
        newErrors.tableCount =
          "El número de mesas es requerido para los servicios seleccionados";
    ...
    if (requiresRoomCount) {
      if (!formData.roomCount || formData.roomCount < 1) {
        newErrors.roomCount =
          "El número de habitaciones es requerido para Room Service";
  ...
  const handleServiceToggle = (serviceId: string) => {
    setFormData((prev) => {
      ...
      const requiresTableCount =
        newServices.includes("flex-bill") ||
        newServices.includes("tap-order-pay");
      ...
      <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6 relative">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
          <div>
            ...
            <input
              type="text"
              className={`w-full px-2 sm:px-3 py-1.5 sm:py-2 text-sm border-2 focus:outline-none focus:border-[#023828] ${
                errors.name ? "border-red-300" : "border-[#023828]/30"
              }`}
              value={formData.name}
              onChange={(e) =>
          ...
          <div>
            ...
            <input
              type="text"
              className={`w-full px-2 sm:px-3 py-1.5 sm:py-2 text-sm border-2 focus:outline-none focus:border-[#023828] ${
                errors.ownerName ? "border-red-300" : "border-[#023828]/30"
              }`}
          ...
          <div>
            ...
            <input
              type="tel"
              className={`w-full px-2 sm:px-3 py-1.5 sm:py-2 text-sm border-2 focus:outline-none focus:border-[#023828] ${
                errors.phone ? "border-red-300" : "border-[#023828]/30"
              }`}
          ...
          <div>
            ...
            <input
              type="email"
              className={`w-full px-2 sm:px-3 py-1.5 sm:py-2 text-sm border-2 focus:outline-none focus:border-[#023828] ${
                errors.email ? "border-red-300" : "border-[#023828]/30"
              }`}
                ...
                <div className="flex items-start">
                  <div className="text-xs sm:text-sm">
                    ...
                    <p className="text-yellow-700 mb-2 text-[10px] sm:text-sm break-all">
                      {emailChangeWarning.oldEmail} →{" "}
                      {emailChangeWarning.newEmail}
                    </p>
                  </div>
                </div>
              </div>
        ...
        {(formData.services.includes("flex-bill") ||
          ...
          <div>
            ...
            <input
              ...
              max="500"
              className={`w-full px-2 sm:px-3 py-1.5 sm:py-2 text-sm border-2 focus:outline-none focus:border-[#023828] ${
                errors.tableCount ? "border-red-300" : "border-[#023828]/30"
              }`}
        ...
        {formData.services.includes("room-service") && (
          <div>
            ...
            <input
              ...
              max="500"
              className={`w-full px-2 sm:px-3 py-1.5 sm:py-2 text-sm border-2 focus:outline-none focus:border-[#023828] ${
                errors.roomCount ? "border-red-300" : "border-[#023828]/30"
              }`}
            ...
            <p className="mt-1 text-[10px] sm:text-xs text-yellow-700">
              <strong>
                Requerido para Room Service - Servicio a la habitación
              </strong>
            </p>
          </div>
        )}
        ...
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-3">
            {AVAILABLE_SERVICES.map((service) => (
              <label
                key={service.id}
                className="flex items-start space-x-2 sm:space-x-3 p-2 sm:p-3 border border-[#023828]/20 hover:bg-[#C3FEFF] cursor-pointer transition-colors"
              >
                <input
                  type="checkbox"
                  className="h-4 w-4 sm:h-5 sm:w-5 text-[#023828] border-[#023828]/30 focus:ring-[#82E657] mt-0.5"
        ...
        <div className="space-y-3 sm:space-y-4">
          <label className="flex items-center">
            <input
              type="checkbox"
              className="h-4 w-4 sm:h-5 sm:w-5 text-[#023828] border-[#023828]/30 focus:ring-[#82E657]"
          ...
          {!client && (
            <div className="bg-[#C3FEFF]/30 border border-[#023828]/20 p-3 sm:p-4">
              <div className="flex items-start">
                <input
                  type="checkbox"
                  className="h-4 w-4 sm:h-5 sm:w-5 text-[#023828] border-[#023828]/30 mt-0.5"
                  checked={sendInvitation}
                  readOnly
                  disabled
                />
                <div className="ml-2 sm:ml-3">
                  <span className="text-xs sm:text-sm font-medium text-[#023828]">
                    Invitación automática por email
                  </span>
                  <p className="text-[10px] sm:text-xs text-[#023828]/60 mt-1">
